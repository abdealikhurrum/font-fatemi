...
final class DiacriticOverlayPanel: NSPanel {
    ...
    private static func makeOptionAttrString() -> NSAttributedString {
        ...
        b.section("DASHES & SYMBOLS")
        b.row2("\u{2325}-", "\u{2013}", "en dash \u{2013}",         "\u{2325}=", "\u{2014}", "em dash \u{2014}")
        b.row2("\u{2325}Z", "\u{2015}", "horizontal bar \u{2015}",  "\u{2325}`", "\u{0640}", "tatweel \u{0640}")
        b.row2("\u{2325}X", "\u{00B0}", "degree \u{00B0}",           "\u{2325}C", "\u{2122}", "trademark \u{2122}")
        b.row2("\u{2325}V", "\u{00B1}", "plus-minus \u{00B1}",       "\u{2325}B", "\u{00D7}", "multiply \u{00D7}")
        b.row2("\u{2325}N", "\u{00F7}", "divide \u{00F7}",           "\u{2325}M", "\u{00B7}", "middle dot \u{00B7}")
        b.row2("\u{2325}J", "\u{00A9}", "copyright \u{00A9}",        "\u{2325}K", "\u{00AE}", "registered \u{00AE}")
        b.spacer()

        b.section("QUOTATION MARKS  (⌥ + QWERTY row)")
        b.row2("\u{2325}Q", "\u{2018}", "left single \u{2018}",      "\u{2325}W", "\u{2019}", "right single \u{2019}")
        b.row2("\u{2325}E", "\u{201C}", "left double \u{201C}",      "\u{2325}R", "\u{201D}", "right double \u{201D}")
        b.row2("\u{2325}U", "\u{2039}", "guillemet \u{2039}",        "\u{2325}I", "\u{203A}", "guillemet \u{203A}")
        b.row2("\u{2325}O", "\u{00AB}", "guillemet \u{00AB}",        "\u{2325}T", "\u{2026}", "ellipsis \u{2026}")
        b.row2("\u{2325}[", "\u{FD3E}", "Arabic ornate \u{FD3E}",    "\u{2325}]", "\u{FD3F}", "Arabic ornate \u{FD3F}")
        b.row1("\u{2325}Y", "\u{2022}", "bullet \u{2022}")
        b.spacer()

        b.section("BRACKETS  (⌥ + ASDF row)")
        b.row2("\u{2325}A", "{", "curly {",   "\u{2325}S", "}", "curly }")
        b.row2("\u{2325}D", "[", "square [",  "\u{2325}F", "]", "square ]")
        b.row2("\u{2325}G", "<", "angle <",   "\u{2325}H", ">", "angle >")
        b.spacer()

        b.section("ARABIC PUNCTUATION")
        b.row2("\u{2325},",   "\u{060C}", "Arabic comma \u{060C}",       "\u{2325};",   "\u{061B}", "Arabic semicolon \u{061B}")
        b.row2("\u{2325}/",   "\u{061F}", "Arabic ? \u{061F}",           "\u{2325}Spc", "\u{00A0}", "non-breaking space")
        b.spacer()

        b.section("SUBTENDING MARKS  (⌥ + key, then type digits)")
        b.row2("\u{2325}L", "\u{0601}", "sanah \u{2014} year sign",  "\u{2325}P", "\u{0603}", "safha \u{2014} page sign")
